<?php
	function db_connect(){
		...
		$row = array();
		$query = "SELECT book_isbn, book_image, book_price, book_title, publisherid FROM books ORDER BY book_isbn DESC";
		...
		$query = "SELECT book_title, book_author, book_price, pdf FROM books WHERE book_isbn = '$isbn'";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
	...
	function getCatName($conn, $catid){
		$query = "SELECT categery_name FROM catogeries WHERE categeryid = '$catid'";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
	...
	function insertIntoOrder($conn, $customerid, $total_price, $date, $ship_name, $ship_address, $ship_city, $ship_zip_code, $ship_country){
		...
		if(mysqli_num_rows($result) == 0){
			echo "Empty books ! Something wrong! check againn";
			exit;
		}
